...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{LaTeX_ThesisEnglish/Images/synethic_data_pipeline.png}
    \caption{For this project, the \textbf{Synthetic (On-the-fly) Generation} method was adopted. All models are trained using LR patches dynamically generated from the HR images, eliminating the need for additional LR datasets.}
...
This is the classic 4-level encoder-decoder with symmetric skip connections. Each encoder block consists of a Conv2D $\rightarrow$ BatchNorm $\rightarrow$ Relu and then halves the spatial resolution with MaxPool2D. Each decoder block consists of UpSampling2D (with bilinear interpolation) $\rightarrow$ Conv2D $\rightarrow$  concatenation of the corresponding skip connections. The head is a single 1×1  convolution (with sigmoid activation) that directly predicts the HR RGB patch, the complete architecture is illustrated in Figure \ref{fig:vanilla_unet}.

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{LaTeX_ThesisEnglish/Images/U-Net_vanillin.png}
    \caption{U-Net architecture 4 level encoder-decoder}
    \label{fig:vanilla_unet}  
\end{figure}
